...
//   const offset = helper.getOffset(page, config.listPerPage);
  const screenplayreviews = await Screenplayreview.find().limit(25);
  return screenplayreviews
}

async function getSingle(id) {
    const screenplayreview = await Screenplayreview.findById(id)
        .populate("writer");
...
async function update(id, newScreenplayreview){
    const result = await Screenplayreview.findByIdAndUpdate(id, newScreenplayreview);
    return result
}
