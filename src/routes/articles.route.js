...
router.post(
  "/uploadImage/writer/:writerId",
  [jwtCheck, upload.single("file")],
  Article.uploadArticleImage
);
router.get("/getImageUrls/writer/:writerId", jwtCheck, Article.getBucketUrls);
router.post(
  "/uploadProfileImage/writer/:writerId",
  [jwtCheck, upload.single("file")],
  Article.uploadProfileImage
);
// router.get('/test', Article.getArticleById)
router.post("/evaluate", jwtCheck, Article.evaluateArticle);
router.get(
  "/getJobStatus/:jobId/:queueType",
  jwtCheck,
  Article.getEvalJobStatus
);
