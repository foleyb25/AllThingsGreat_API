...
createQueue.process(async (job) => {
  ...
  const uniqueNumber = Math.floor(10000000 + Math.random() * 90000000);
  // Normalize the title by converting it to lowercase and replacing spaces with hyphens
  const slugTitle = slugify(job.data.article.title, {
    replacement: "-", // replace spaces with replacement
    remove: /[*+~.()'"!:@]/g, // regex to remove characters
    lower: true, // result in lower case
  });
  ...
  job.data.article.estimatedReadTime = Number(
    calculateEstimatedReadTime(job.data.articleText)
  );
